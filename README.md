...
- One-click LLM analysis for staged files (async OpenAI-compatible tool loop: `read_file`, `load_media`, `rename_file`, `mark_text`, `mark_image`, `set_datetime`, `set_title`, `finish`). Each staged file is pre-converted toward the two model-readable forms — paged text and normalized images — written into `.filestor/analyze/` under source-derived names (`name.txt`, `name.pNN.jpg` pages, `name.fN.jpg` video frames, `name.jpg`); the initial prompt lists each source with its converted forms (plus line counts and a one-line peek for native text) so the model can plan its own path, e.g. falling back to a document's page images when the text form is empty. `read_file` pages through text by line offset/limit; `load_media` loads all derived images of a source at once. The run ends when the model calls `finish` (stopping without it earns a nudge to continue or finish); if the budget runs out, a forced wrap-up round with read tools closed replaces failure. With `mark_text` the model splits the important text into small, independently indexable chunks, and with `mark_image` it picks the content-bearing images not already covered by those chunks — the batch's content digest for later search/embedding, written as individual files under `.filestor/digest/` (one `text-NN.txt` per chunk, one `image-NN-<name>` per image), rebuilt by every run and dropped when the staged set changes. At push time, with `llm.embeddings` and `llm.vectors` both configured, each digest file is embedded through the multimodal embeddings endpoint (one request per file) and the vectors are written into the OSS Vectors index keyed by bundle id, before the bundle's meta and monthly index are published. The model may also rename clearly misnamed staged files (never overwriting another file, derived forms renamed along); it works in a fixed order — inspect, rename, mark, title/datetime — and a push then uses the new names. While the LLM is configured, staged files must pass one successful analysis before they can be pushed; adding or deleting files resets that and disables the upload button until the next analysis.
...
All of `admin.username`, `admin.password`, and `s3.{endpoint,region,bucket,access_key_id,secret_access_key}` are required; `s3.force_path_style` is optional (needed by MinIO-style vendors). Any S3-compatible storage works — Aliyun OSS (use its S3-compatible endpoint `https://s3.oss-{region}.aliyuncs.com`), Qcloud COS (`https://cos.{region}.myqcloud.com`), AWS S3, MinIO, etc. `endpoint` may omit `https://`; it is added on load. `upload.workspace` defaults to `upload-workspace` (relative to the process working directory; in the container that is `/upload-workspace`). `llm.chat.openai.{url,model,effort,headers}` is an optional OpenAI-compatible chat endpoint used by file analysis on `/upload`; `url` and `model` must be set together, `effort` is the model's reasoning effort, and `headers` is an optional map of extra HTTP headers (e.g. `Authorization`). `llm.embeddings.bailian_multimodal_embedding.{url,model,headers,dimensions}` is an optional, independent embeddings endpoint (no `chat` defaults); `url` and `model` must be set together and `dimensions` is optional. `llm.vectors.aliyun_oss_vectors.{url,access_key_id,access_key_secret,account_id,index}` is an optional, independent vector store (no `embeddings` defaults); `url` includes the bucket host in the shape `<bucket>.<region>.oss-vectors.aliyuncs.com`, `account_id` is the Alibaba Cloud account that owns the vector bucket (used in the OSS Vectors V4 signature), and `url`, `access_key_id`, `access_key_secret`, `account_id`, and `index` must be set together. Every `llm.*.url` must carry an `http(s)://` scheme. Do not commit `config.yaml`.
...
Workflow: `.github/workflows/release.yml`. The image includes LibreOffice, ImageMagick, poppler-utils, pandoc, catdoc, ffmpeg, and Noto CJK fonts so file analysis can pre-convert documents to text and page images, normalize odd or oversized images, and extract video frames without changing staged files. Local `go run` still reads native text and small jpeg/png/gif without those tools.
