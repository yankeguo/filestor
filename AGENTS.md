...
- **`/upload` is a local staging directory only.** `GET /upload` + `GET|POST|DELETE /upload/files`. Write files under `upload.workspace` (create the dir if missing). List only regular files in the workspace root; skip subdirectories and `.upload-*` temp files. Sanitize names with `path.Base`; reject `.`, `..`, empty, and `.upload-*`. Do not PUT to OSS from this page. Do not serve workspace bytes. Multipart field is `file`; max request size 2 GiB.
